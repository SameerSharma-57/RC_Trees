...
void mark_compressed_vertices(const CSR_mat old_mat, int *c, Vertex *d,
                              const Vertex round)
{
...
__global__ void find_2_degree_kernel(int *c, const Vertex vertices,
                                     const Vertex *d, const Vertex *cct,
                                     const Vertex *idx)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void mark_compressible_nodes(int *c, const Vertex vertices,
                                        const Vertex *d, const Vertex *cct,
                                        const Vertex *idx)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
...
// __global__ void compress(const CSR_mat &old, CSR_mat &new_mat, const Vertex*
// d, const Vertex vertices, const bool* c ){
