...
__global__ void indFind(int N, int* SubGraph, unsigned int* Random, Vertex* cct,Vertex* idx) {
	...
	if (found && SubGraph[u] && SubGraph[v]) {	//may not be valid in a moment
...
__global__ void updateWithNeighs(int N, int* Graph, int* Adj, int* cct, int* idx,  int* IndSet) {
	//each thread handles one edge
